...
public class BuildDetailsHibernateRepositoryTests {
    ...
    private final BuildDetailsRepository repository = new BuildDetailsHibernateRepository( HibernateUtil.getSessionFactory(HibernateUtil.getExtraProperties(TestUtils.JDBC_DRIVER, TestUtils.JDBC_URL, TestUtils.JDBC_USER, TestUtils.JDBC_PASS)));

    final HibernateTransactionManager txmgr = ((BuildDetailsHibernateRepository) repository).getTransactionManager();
    ...
    public void retrievalByMatchingFullNameShouldReturnNonEmptyList() {
      ...
      final List<BuildDetails> builds = repository.getBuildDetailsByFullName(build.getFullName().toLowerCase());
      Assert.assertNotNull("Unexpected null list of builds", builds);
      Assert.assertFalse("Unexpected empty list of builds", builds.isEmpty());
      Assert.assertEquals("Unexpected number of builds", 1, builds.size());
      Assert.assertEquals("Mismatching build details found", build, builds.get(0));
    ...
    public void retrievalByMatchingNameShouldReturnNonEmptyList() {
      ...
      final List<BuildDetails> builds = repository.getBuildDetailsByName(build.getName().toLowerCase());
      Assert.assertNotNull("Unexpected null list of builds", builds);
      Assert.assertFalse("Unexpected empty list of builds", builds.isEmpty());
      Assert.assertEquals("Unexpected number of builds", 1, builds.size());
      Assert.assertEquals("Mismatching build details found", build,builds.get(0));
    ...
    public void retrievalByNonMatchingUserNameShouldReturnEmptyList() {
      ...
      final List<BuildDetails> builds = repository.getBuildDetailsByUserName(build.getUserName() + "NOMATCH");
      Assert.assertNotNull("Unexpected null list of builds", builds);
      Assert.assertTrue("Unexpected non-empty list of builds", builds.isEmpty());
    }
